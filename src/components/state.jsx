    ...
    <div className="state-subcontainer">
        <div className="state-label-wrapper">
            <div className="state-label">
                ...
                <div className="stateIcon">
                    ...
                    : storage.length !== 0 && storage[index].fatality_rate >= .0001 ? <>
                                <svg width="17" height="17" viewBox="0 0 27 28" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="stateWrapper">
            <div className="state-container-card">
                {/* refact to include data-cy=" 'color'-status"  */}
            {storage.length !== 0 && storage[index].fatality_rate >= .02 ? <> Why is <span id="red">{storage[index].region.province}</span> under<br/> current status?</>
            : storage.length !== 0 && storage[index].fatality_rate >= .01 ? <> Why is <span id="yellow">{storage[index].region.province}</span> under<br/> current status?</>
            : storage.length !== 0 && storage[index].fatality_rate >= .0001 ? <> Why is <span id="green">{storage[index].region.province}</span> under<br/> current status?</>
            : <div></div>}
            </div>
            <div className="state-container-information">
                <ul>
                    ...
                    <li>
                        ...
                        <div className="state-container-information-right-orange">{storage[index].deaths}</div>
                    </li>
                    <li>
                        <div className="state-container-information-left">Fatality Rate:</div>
                        <div className="state-container-information-right-orange">{storage[index].fatality_rate}</div>
                    </li>
                    <li>
                        <div className="state-container-information-left">Confirmed Cases:</div>
                        <div className="state-container-information-right-red">{storage[index].confirmed}</div>
                    </li>
                    <li>
                        <div className="state-container-information-left">Active Cases:</div>
                        <div className="state-container-information-right-red">{storage[index].active}</div>
                    </li>
                    <li>
